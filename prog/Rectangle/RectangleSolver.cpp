...
int main(int argc, char **argv)
{
  ...
  int max_depth     = 12; // number of pieces
  int ids_max       = 12; // number of pieces
  int generation_limit = 0;
  int store_limit = 0;
  bool show_solution = false;
  
  while((c = getopt(argc, argv,"f:g:l:L:m:St:x:y:h")) != EOF)
    {
      switch(c)
        {
        ...
        case 'x': board_width      = atoi(optarg); break;
        case 'y': board_height     = atoi(optarg); break;
  ...
  if(show_help)
    {
      ...
      std::cout << "-x num   : puzzle width" << std::endl;
      std::cout << "-y num   : puzzle height" << std::endl;
